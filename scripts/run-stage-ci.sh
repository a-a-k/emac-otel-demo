...
go run ./cmd/emacctl stage-plan --seed "$EMAC_RUN_SEED" --run "feasibility-${GITHUB_RUN_ID}" --stage "$EMAC_WEIGHT" --weight "$weight_fraction" --warmup 200 --measured "$EMAC_N_MAX" --out "$EMAC_RESULTS/stage-plan.json"
